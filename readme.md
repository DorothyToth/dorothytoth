...
In order to properly publish this content to a webserver, however, we must run the `pnpm sync-project-assets` that will pull all `jpg`, `jpeg`, `png`, `pdf`, and `gif` files from these project directories and push them to the matching directory in `/public/projects` where the server serves them.  
